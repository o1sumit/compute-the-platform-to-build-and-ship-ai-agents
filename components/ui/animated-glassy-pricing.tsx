...
export const ShaderCanvas = () => {
  ...
  return (
    <canvas
      ref={canvasRef}
      className="absolute inset-0 w-full h-full block z-0 opacity-45 dark:opacity-30 pointer-events-none"
    />
  );
};
...
export const PricingCard = ({
  ...
  const cardClasses = `
    ...
    from-white/5 to-white/0 border border-black/10
  ...
  `;
  const buttonClasses = `
    mt-auto w-full py-2.5 rounded-xl font-semibold text-[14px] transition font-sans flex items-center justify-center
    ${
      buttonVariant === "primary"
        ? "bg-cyan-400 hover:bg-cyan-300 text-neutral-900 shadow-lg shadow-cyan-400/20 hover:shadow-cyan-400/30"
  ...
  return (
    <div className={cardClasses.trim()}>
      {isPopular && (
        <div className="absolute -top-4 right-4 px-3 py-1 text-[12px] font-semibold rounded-full bg-cyan-400 text-neutral-950">
          Most Popular
        </div>
      )}
      <div className="mb-3">
        ...
        <p className="text-[14px] text-muted-foreground mt-1 font-sans">{description}</p>
      </div>
      ...
      <RippleButton
        ...
        onClick={onClick}
        variant={buttonVariant === "primary" ? "default" : "hover"}
        rippleColor={buttonVariant === "primary" ? "rgba(255,255,255,0.35)" : undefined}
      >
        {buttonText}
      </RippleButton>
    </div>
