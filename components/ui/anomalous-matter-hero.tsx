...
function readMeshColor(isDark: boolean): THREE.Color {
  if (typeof window === "undefined") {
    return new THREE.Color(isDark ? "#60c8ff" : "#3b82f6");
  }

  const root = document.documentElement;
  const raw = getComputedStyle(root).getPropertyValue("--sky-300").trim();
  if (raw) {
    const color = new THREE.Color();
    color.setStyle(`hsl(${raw.replace(/\s+/g, ", ")})`);
    return color;
  }
...
export function GenerativeArtScene({ className, meshOffsetX = 0 }: GenerativeArtSceneProps) {
  ...
  useEffect(() => {
    ...
    const material = new THREE.ShaderMaterial({
      uniforms: {
        ...
        color: { value: readMeshColor(isDark) },
      },
  ...
  }, [resolvedTheme, meshOffsetX]);

  return <div ref={mountRef} className={className ?? "absolute inset-0 w-full h-full z-0"} />;
}
