    ...
    const animate = (currentTime: number) => {
      ...
      switch (animationState) {
        ...
        case "vaporizing": {
          ...
          if (vaporizeProgressRef.current >= 100 && allVaporized && !isTransitioningRef.current) {
...
const createParticles = (
  ...
  if (minX > maxX) {
    minX = textBoundaries.left;
    maxX = textBoundaries.right;
  }

  const actualTextBoundaries = {
    left: minX,
    right: maxX,
  ...
  return { particles, textBoundaries: actualTextBoundaries };
};
  ...
  if (typeof document !== "undefined") {
    ...
    if (ctx) {
      ...
      if (computed.startsWith("#")) {
        ...
        return computed.replace("rgb", "rgba").replace(")", ", 1)");
      }
    }
  }
